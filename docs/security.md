...
Codex 只能通过应用数据目录中的受控工具链桥使用当前项目已经选择的工具。桥目录带 Research IDE 所有权标记并拒绝符号链接重定向；别名是应用生成的参数透传包装器，不来自项目。自定义和托管可执行文件在生成桥前再次核对规范路径及指纹，托管包装器只携带已验证的 `PATH`、`CONDA_PREFIX`、`CONDA_DEFAULT_ENV` 和 `CONDA_SHLVL`。桥中不生成 Pixi、安装、更新、移除或版本切换入口，线程职责提示也明确禁止这些生命周期操作。
...
Codex CLI 本身也是供应链边界。系统探测和手动导入拒绝项目内可执行文件；手动选择记录路径、版本与 SHA-256，解析时逐次复核。托管下载只接受 `api.github.com/repos/openai/codex/releases` 返回的稳定版、匹配当前平台的精确资产、OpenAI GitHub release URL、声明大小和 GitHub 提供的不可变 SHA-256 digest；重定向限制在 GitHub release 资产主机，响应与解压体积均有上限。tar 解包只接受一个预期名称的普通文件，拒绝链接、额外文件和路径穿越。`codex-runtime` 根、版本目录、manifest 和选择记录都在 app-owned marker 与规范路径检查后访问；新版本完整验证后才原子切换，任何失败保留原选择和系统安装。
...
安装升级身份属于安全边界。Windows Squirrel package/AppUserModelId、macOS bundle id 和 Linux package name 必须保持稳定并由发布门禁复核；安装 manifest 记录同一身份与应用版本。应用只在带自身 marker 的 `userData` 中记录上次运行版本和 Windows install/update 事件，不扫描项目、home 或系统应用目录来猜测旧版本。版本状态目标若是符号链接或非普通文件则拒绝替换；写入使用同目录独占临时文件、`fsync`、目录身份复核和 rename。安装器替换应用文件不授权迁移、删除或上传科研项目。
